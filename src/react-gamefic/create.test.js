...
describe('create', () => {
  it('configures webpack', () => {
    const webpack = fs.readFileSync(path.join(directory, 'webpack.config.cjs')).toString();
    expect(webpack.includes('"../lib"')).toBeTruthy();
  });

  it('configures index.html', () => {
    const index = fs.readFileSync(path.join(directory, 'public', 'index.html')).toString();
    expect(index.includes(options.name)).toBeTruthy();
  });

  it('configures manifest.json', () => {
    const manifest = fs.readFileSync(path.join(directory, 'public', 'manifest.json')).toString();
    expect(manifest.includes(options.name)).toBeTruthy();
  });

  it('configures package.json', () => {
    const pack = fs.readFileSync(path.join(directory, 'package.json')).toString();
  ...
  it('configures index.tsx', () => {
    const index = fs.readFileSync(path.join(directory, 'src', 'index.tsx')).toString();
    expect(index.includes(options.name)).toBeTruthy();
  })

  it('configures driver.tsx', () => {
    const driver = fs.readFileSync(path.join(directory, 'src', 'driver.tsx')).toString();
