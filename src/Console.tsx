  ...
  useEffect(() => {
    if (!started) {
      ...
      driver.onUpdate((output: OutputType) => {
        setOutputs((history: OutputType[]) => [
          ...history.slice(history.length + 1 - historySize),
          output,
        ]);
        setIsLoading(false);
  ...
  useEffect(() => {
    if (outputs.length > 1) {
      ...
      if (lastSnapshot && !getOutput()._metaFromConsole) {
        const undoSavePoints: string[] = JSON.parse(
          window.sessionStorage.getItem(undoSavePointsKey) || "[]",
        );
        undoSavePoints.push(lastSnapshot);
        window.sessionStorage.setItem(
          undoSavePointsKey,
          JSON.stringify(
            undoSavePoints.slice(undoSavePoints.length + 1 - undoSize),
          ),
        );
      }
    }
    ...
    return (
      <GameContext.Provider value={context}>
        <div className={className}>
          ...
          <div
            ref={bottomRef}
            data-scene-name={context.output?.scene.name}
            data-scene-type={context.output?.scene.type}
          />
        </div>
